...
fn main() -> Result<()> {
    let mut packages = bad_packages_list();
    get_dates(&mut packages)?;
    let output = serde_json::to_string_pretty(&packages)?;
    fs::write("bad-packages.json", output)?;
    Ok(())
}

fn get_dates(packages: &mut BTreeMap<String, BadPackage>) -> Result<()> {
    for package in packages.iter_mut() {
        let url = format!("https://registry.npmjs.com/{}/", package.0);
        println!("Getting: {}", &url);
        let overview = get_json(&url)?;
        let times = &overview.get("time").ok_or(anyhow!("Could not find time"))?;
        for version in &mut package.1.versions {
            version.date = times
                .get(&version.number)
...
fn bad_packages_list() -> BTreeMap<String, BadPackage> {
    let mut bad_packages = BTreeMap::new();

    let text = r#"@ahmedhfarag/ngx-perfect-scrollbar	20.0.20
...
yoo-styles	6.0.326"#;
    ...
    // let text = "@art-ws/config-eslint	2.0.4, 2.0.5";
    ...
    text.lines().for_each(|line| {
        ...
        let versions = parts[1]
            .split(",")
            .map(|version| Version {
                number: version.to_string().trim().to_string(),
                date: None,
            })
            .collect::<Vec<Version>>();
        bad_packages.insert(parts[0].clone(), BadPackage { versions });
    });
    bad_packages
}
